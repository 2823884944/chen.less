...
public class RedisRaterLimiter {
    ...
    private static final String LIMIT_MAX_PRE = "C_LIMIT_MAX:";

    private static final String LIMIT_NOW_PRE = "C_LIMIT_NOW:";
    ...
    public boolean getLimitFlag(@ApiParam(name = "IP地址等唯一标识") String point, @ApiParam(name = "限流次数") Integer limit, @ApiParam(name = "限流单位时间") Long timeout) {
        ...
        try {
            ...
            if(CsxNullUtils.isNull(maxCount)){
                // 重置次数
                ...
                return true;
            } else if (!CsxNullUtils.isNull(maxCount) && !CsxNullUtils.isNull(currCount)){
