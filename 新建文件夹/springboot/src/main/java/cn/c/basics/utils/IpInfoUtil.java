    ...
    public String getIpCity(HttpServletRequest request){
        ...
        try {
            ...
            if(Objects.equals(objectResult.get("status").getAsString(),"0")) {
                ...
                if(!CsxNullUtils.isNull(nationStr) && CsxNullUtils.isNull(provinceStr)){
                    resultStr = nationStr;
                } else {
                    resultStr = provinceStr;
                    if(!CsxNullUtils.isNull(cityStr)){
                        resultStr += "-" + cityStr;
                    }
                    if(!CsxNullUtils.isNull(districtStr)){
