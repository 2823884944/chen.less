...
<div class="search">
    <Card>
        ...
        <Row :gutter="16">
            ...
            <Input v-model="searchKey" suffix="ios-search" @on-change="search" placeholder="输入组织名搜索" clearable />
            ...
            <Form ref="form" :model="form" :label-width="100" :rules="formValidate">
                <Row :gutter="16">
                    <Col span="12">
                    <FormItem label="上级组织" prop="parentTitle">
                        <div style="display:flex;">
                            <Input v-model="form.parentTitle" readonly style="margin-right:10px;" />
                            <Poptip transfer trigger="click" placement="right-start" title="选择上级组织" width="250">
                                <Button icon="md-list">选择组织</Button>
                ...
                <Row :gutter="16">
                    <Col span="12">
                    <FormItem label="组织领导" prop="mainHeader">
                        <Select :loading="userLoading" not-found-text="该组织暂无用户数据" v-model="form.mainHeader" multiple filterable placeholder="选择组织领导">
                    ...
                    <Col span="12">
                    <FormItem label="组织组长" prop="viceHeader">
                        <Select :loading="userLoading" not-found-text="该组织暂无用户数据" v-model="form.viceHeader" multiple filterable placeholder="选择组织组长">
                            <Option v-for="item in users" :value="item.id" :key="item.id" :label="item.nickname">
                                <span style="margin-right:10px;">{{ item.nickname }}</span>
                                <span style="color:#ccc;">{{ item.username }}</span>
                            </Option>
    ...
    <Divider dashed>组织下用户列表</Divider>
    <Card>
        <Row>
    ...
    <Modal :title="modalTitle" v-model="addOrEditDepartmentModal" :mask-closable="false" :width="500" on-text="提交" @on-ok="submitAdd">
        <Form ref="formAdd" :model="formAdd" :label-width="85" :rules="formValidate">
            ...
            <Row :gutter="16" v-show="showParent">
                <Col span="24">
                <FormItem label="上级组织：">{{form.title}}</FormItem>
                </Col>
            </Row>
            <Row :gutter="16">
                <Col span="24">
    ...
    methods: {
        ...
        submitEdit() {
            this.$refs.form.validate(valid => {
                if (valid) {
                    if (!this.form.id) {
                        this.$Message.warning("请先点击选择要修改的组织");
                        return;
                    }
